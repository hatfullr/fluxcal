     ...
     * iwrk,kwrk,ier)
c      subroutine bispev(tx,nx,ty,ny,c,csize,kx,ky,x,mx,y,my,z,wrk,lwrk,
c     * iwrk,kwrk,ier)
...
c
c  ..scalar arguments..
      integer nx,ny,kx,ky,mx,my,lwrk,kwrk,ier,csize
c  ..array arguments..
      integer iwrk(kwrk)
      real tx(nx),ty(ny),c((nx-kx-1)*(ny-ky-1)),x(mx),y(my),z(mx*my),
     * wrk(lwrk)
c      real tx(nx),ty(ny),c(csize),x(mx),y(my),z(mx*my),
c     * wrk(lwrk)
c  ..local scalars..
      integer i,iw,lwest
c  ..
