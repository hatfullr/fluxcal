...
!     .. Parameters ..
      ...
      REAL*8, INTENT(IN)      :: xd(nxd)
      REAL*8, INTENT(IN)      :: yd(nyd)
      REAL*8, INTENT(IN OUT)  :: zd(nxd,nyd)
      INTEGER, INTENT(IN)   :: nip
      REAL*8, INTENT(IN OUT)  :: xi(nip)
      REAL*8, INTENT(IN OUT)  :: yi(nip)
      REAL*8, INTENT(IN OUT)  :: zi(nip)
...
!     Format statements for error messages
 ...
 9050 FORMAT ('    NXD =', i5,',  NYD =', i5,',  NIP =', i5/)
      ...
      SUBROUTINE rgsf3p(md, nxd, nyd, xd, yd, zd, nxi, xi, nyi, yi, zi,
     $     ier)
...
!     .. Parameters ..
      ...
      REAL*8, INTENT(IN OUT)  :: xi(nxi)
      INTEGER, INTENT(IN)   :: nyi
      REAL*8, INTENT(IN)      :: yi(nyi)
      REAL*8, INTENT(IN OUT)  :: zi(nxi,nyi)
      INTEGER, INTENT(OUT)  :: ier
...
!    $     (zz1-zz0)* (xx3-xx2)/xx1)* (xx3/ (xx2-xx1)) + zz0
      ...
      REAL*8, INTENT(IN)  :: xx1, xx2, zz0, zz1
      REAL*8              :: fn_val
      ...
      REAL*8, INTENT(IN)  :: xx1, xx2, xx3, zz0, zz1, zz2
      REAL*8              :: fn_val
...
!     .. Scalar Arguments ..
      ...
      REAL*8, INTENT(IN)     :: xd(nxd)
      REAL*8, INTENT(IN)     :: yd(nyd)
      REAL*8, INTENT(IN)     :: zd(nxd,nyd)
      REAL*8, INTENT(OUT)    :: pdd(3,nxd,nyd)
...
!     .. Local Scalars ..
     ...
     $     z23, z30, z31, z32, z33, zxdi, zxydi, zydi
      INTEGER :: ipex, ipey, ix0, ix1, ix2, ix3, iy0, iy1, iy2, iy3,
     $     nx0, ny0
!     ..
!     .. Local Arrays ..
      REAL*8    :: b00xa(4), b00ya(4), b01a(4), b10a(4), cxa(3,4),
     $     cya(3,4),sxa(4), sxxa(4), sya(4), syya(4), xa(3,4), ya(3,4),
     $     z0ia(3,4), zi0a(3,4)
!     INTEGER :: idlt(3,4)
!     ..
!     .. Intrinsic Functions ..
...
!     and y directions for the primary estimate of zxy.
               ...
               sxyy = sx*syy
               sxxyy = sxx*syy
               sxyz = x1* (y1*z11+y2*z12+y3*z13) + x2*
     $              (y1*z21+y2*z22+y3*z23) +x3* (y1*z31+y2*z32+y3*z33)
...
!     .. Scalar Arguments ..
      ...
      REAL*8, INTENT(IN)      :: yd(nyd)
      INTEGER, INTENT(IN)   :: nip
      REAL*8, INTENT(IN)      :: xi(nip)
      REAL*8, INTENT(IN)      :: yi(nip)
...
!     Determines IYD for which YII lies between YD(IYD) and YD(IYD+1).
 ...
 20         IF (yii >= yd(imd)) THEN
      ...
      SUBROUTINE rgplnl(nxd, nyd, xd, yd, zd, pdd, nip, xi, yi, inxi,
     $     inyi, zi)
...
!     .. Scalar Arguments ..
      ...
      REAL*8, INTENT(IN)     :: yd(nyd)
      REAL*8, INTENT(IN)     :: zd(nxd,nyd)
      REAL*8, INTENT(IN)     :: pdd(3,nxd,nyd)
      INTEGER, INTENT(IN)  :: nip
      REAL*8, INTENT(IN)     :: xi(nip)
      REAL*8, INTENT(IN)     :: yi(nip)
      INTEGER, INTENT(IN)  :: inxi(nip)
      INTEGER, INTENT(IN)  :: inyi(nip)
      REAL*8, INTENT(OUT)    :: zi(nip)

!     ..
!     .. Local Scalars ..
      REAL*8 :: a, b, c, d, dx, dxsq, dy, dysq, p00, p01, p02, p03, p10,
     $     p11,p12, p13, p20, p21, p22, p23, p30, p31, p32, p33, q0, q1,
     $     q2,q3, u, v, x0, xii, y0, yii, z00, z01, z0dx, z0dy, z10,z11,
     $     z1dx, z1dy, zdxdy, zii, zx00, zx01, zx0dy, zx10, zx11,
     $     zx1dy, zxy00, zxy01, zxy10, zxy11, zy00, zy01, zy0dx, zy10,
     $     zy11, zy1dx
...
!     x and y directions.
      IF ((ixdi > 0 .AND. ixdi < nxd) .AND. (iydi > 0 .AND. iydi < nyd))
     $     THEN
